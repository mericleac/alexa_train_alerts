...
def get_next_train(color=None, destination=None, station=None):

    data = requests.get(
        url="http://lapi.transitchicago.com/api/1.0/ttarrivals.aspx?key=0fc2e48c0ad84d99923e89e8607776cd&max=1&mapid=40360&outputType=JSON"
    )

    time_to_arrival = (
        datetime.datetime.strptime(data["ctatt"]["eta"][0]["arrT"], "%Y-%m-%dT%H:%M:%S")
...
def get_next_train_intent(intent, session):
    ...
    data = get_next_train(color, destination, station)
...
def set_color_in_session(intent, session):
    ...
    if "Color" in intent["slots"]:
        ...
        speech_output = (
            "I now know your favorite color is "
            + favorite_color
